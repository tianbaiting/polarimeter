...
from .layout import DetectorPlacement, front_face_center, norm, plate_key_for_sector, ray_point_at_y, ray_point_at_z, scaled
...
def _los_occluders_v2(
    ...
    *,
    ...
    for name, shape in build_ports(cfg).items():
        occluders.append((name, shape))
    for name, shape in build_all_plates(cfg).items():
        occluders.append((name, shape))
    for name, shape in build_plate_load_ties(cfg).items():
        occluders.append((name, shape))
    for name, shape in build_stand(cfg).items():
...
def _los_scope_detail(cfg: GeometryConfig) -> str:
    if cfg.clearance.los_scope == "v1_conceptual":
        return (
            "scope=v1_conceptual; "
            "occluders={plates,plate_ties,ports,stand}; "
    ...
    if cfg.clearance.los_scope == "v2_fullpath":
        return (
            ...
            "occluders={chamber_shell,end_modules,ports,plates,plate_ties,target_hardware,stand}; "
...
def _validate_plates(cfg: GeometryConfig, placements: list[DetectorPlacement]) -> SubsystemValidationResult:
    ...
    pose_ok = (
        ...
        and cfg.plate.v1.mount_plane == "xy"
        and cfg.plate.v2.orientation == "vertical"
        and cfg.plate.v2.mount_plane == "xy"
    )
    checks.append(
        SubsystemCheck(
    ...
    checks.append(
        SubsystemCheck(
            ...
            passed=tie_ok,
            detail=(
                f"columns={len(tie_columns)}, top_caps={len(tie_top_caps)}, "
    ...
    checks.append(
        SubsystemCheck(
            ...
            detail=(
                f"margin={cfg.clearance.los_margin_mm:.3f} mm, failures={len(los_failures)}"
                if not los_failures
...
def _validate_detector(
    ...
    for placement in placements:
        ...
        if support_bridge.isNull() or support_bridge.Volume <= 1e-6:
            bridge_ok = False
            bridge_details.append(f"{placement.tag}: bridge volume too small")
    ...
    checks.append(
        SubsystemCheck(
            ...
            detail=(
                f"matrix_pairs={detector_assembly_pairs}, overlap_tol=1e-3 mm^3"
    ...
    checks.append(
        SubsystemCheck(
            name="support_bridge_connected_to_plate",
            passed=bridge_ok,
            detail=(
                "all detector supports include explicit bridge solids to load-bearing plates"
                if bridge_ok
                else "; ".join(bridge_details)
            ),
        )
    )
    ...
    mount_failures: list[str] = []
    for placement in placements:
        plate = _plate_for_sector(cfg, placement.sector_name)
        if plate.mount_plane == "xy":
            anchor = ray_point_at_z(placement.direction, plate.z_mm)
            in_w = abs(anchor.x - plate.offset_x_mm) <= 0.5 * plate.width_mm
            in_h = abs(anchor.y - plate.offset_y_mm) <= 0.5 * plate.height_mm
        elif plate.mount_plane == "xz":
            if abs(placement.direction.y) < 1e-9:
                anchor = ray_point_at_z(placement.direction, plate.z_mm)
            else:
                anchor = ray_point_at_y(placement.direction, plate.offset_y_mm)
            in_w = abs(anchor.x - plate.offset_x_mm) <= 0.5 * plate.width_mm
            in_h = abs(anchor.z - plate.z_mm) <= 0.5 * plate.height_mm
        else:
            raise ValueError(f"unsupported mount_plane {plate.mount_plane!r}")
        if not (in_w and in_h):
            mount_failures.append(
                f"{placement.tag}: anchor=({anchor.x:.1f},{anchor.y:.1f},{anchor.z:.1f}) outside plate envelope"
            )

    checks.append(
        SubsystemCheck(
            name="support_attached_to_loadbearing_plate",
            passed=not mount_failures,
            detail=(
                "all support anchors are inside corresponding load-bearing plate envelopes"
                if not mount_failures
                else "; ".join(mount_failures)
            ),
        )
    )
...
def _validate_stand(cfg: GeometryConfig) -> SubsystemValidationResult:
    ...
    checks.append(
        SubsystemCheck(
            ...
            detail=(
                f"column_d={cfg.stand.plate_tie_column_diameter_mm:.3f}, "
