...
在专题目录运行 \texttt{make} 编译该专题：中间文件保存在各自的 \path{build/}，成稿 PDF 放在 TeX 源码旁，导航中的 [PDF] 直接链接到这些成稿。例如：\texttt{make -C docs/beamline\_info/afterSRC/polarimeter\_rationale}。从仓库根目录运行 \texttt{make -C docs} 可批量编译阅读清单及双语导航；只更新导航用 \texttt{make -C docs guides}，检查链接用 \texttt{make -C docs verify}。导航中间文件保存在 \path{docs/index/build/}，入口仍为 \path{docs/index_zh.pdf} 和 \path{docs/index_en.pdf}。请保持 docs 目录结构，以保留相对链接。
\endgroup
\end{document}
