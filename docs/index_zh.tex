...
\Read{production-zh}{极化产生、输运与运行综述（中文）}提供完整背景；\Read{production-en}{英文简报}用于快速交流。流强问题分别查\Read{primary-beam}{一次束与偏振氘束参数}和\Read{beam-intensity}{SRC--F3 限流记录}。源恢复情况查\Read{pis-status}{PIS 状态资料}；\Read{pis-timeline}{源状态时间线}供按年代检索。
