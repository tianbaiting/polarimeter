...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\sffamily RIKEN 极化氘束：产生、输运与实验交接}
...
\title{\sffamily\color{ink}RIKEN 极化氘束的产生、输运与极化测量\\[5pt]\large 面向 SAMURAI 实验的技术链与 Sekiguchi 团队工作交接}
...
\thispagestyle{empty}
\begin{abstract}
RIKEN 极化氘束的实验能力由三部分共同形成：离子源设定磁子态布居及极化轴，回旋加速器通过单圈取出维持可控自旋相位，极化仪通过已标定的反应分析本领确定束流状态。对 SAMURAI 实验，最终需要给出的是实际反应顶点处、逐源模式且带不确定度的极化，而不是只引用离子源的名义极化率。Sekiguchi 团队的公开工作覆盖核极化技术、极化测量与散射实验系统；承担相应实验支持，需要把源模式接口、参考标定、两站测量、靶点输运和运行质量控制连成可复算的链条。本文从用户提供的共享聊天出发，按原始文献核对事实，附十一幅原始 PDF 局部截图，并同时标注 PDF 页码、文内页码及图表位置。
\end{abstract}
...
\clearpage
\section{Sekiguchi 的作用，以及本实验怎样承担相应工作}

\subsection{公开材料能够确认的贡献}
RIKEN APR 58 的 Three-Body Nuclear Force Laboratory 报告将核极化技术和极化测量技术列为研究方向，记载 FY2024 的离子源准备、加速测试以及 KuJyaku／极化质子靶系统工作，并列 Kimiko Sekiguchi 为该报告时期的负责人。\src{SekiguchiLab2025}{PDF 1／文内 370，Major Research Subjects、Summary、Members}

\screenshot{sekiguchi_lab_p1_members.png}{0.30}{研究组报告的负责人栏原始截图。来源：RIKEN APR 58\cite{SekiguchiLab2025}，单篇 PDF 第 1 页／文内第 370 页，Members。该组织记录不是本次实验的任务书。}{fig:sek-role}

从具体成果看，Sekiguchi 第一作者的 2004 年论文记录了多轴极化、两站 beam-line polarimetry 和绝对标定；2017 年论文提供了 186.6 MeV/u 完整分析本领。她也参与署名 2016 年极化束加速技术报告、2025 年源恢复测试及 2026 年新测量系统论文。因而可确认的作用包括极化散射实验的组织与方法、参考数据和测量系统建设。\cite{Sekiguchi2004,Sekiguchi2017,Sakamoto2016,Sugahara2025,Saito2026}

PIS 的早期设备论文与加速器技术报告同时显示设施能力来自多个团队；不能把源硬件开发、回旋加速器操作、束流线安装权限都归为个人职责。对使用非极化 Sn 靶的束流实验，承担束流极化支持并不要求复制用于双极化 $d$--$p$ 实验的 triplet-DNP、NMR 或 KuJyaku。\src{Ikegami1993}{PDF 1，作者与 Introduction}\src{Saito2026}{PDF 3--4，2.1--2.3}

\subsection{面向交接的任务与交付物}
下表把“cover”解释为能独立执行实验侧工作、明确设施接口并交付可复查结果。它是本报告提出的工作分解，\textbf{不是 Sekiguchi 已承诺承担或移交的正式清单}。

\begin{table}[H]\centering\small
\begin{tabularx}{\linewidth}{p{28mm}YY}\toprule
工作 & 实验侧应交付 & 应与设施／设备方确认 \\\midrule
模式与极化轴 & 目标 $P,T,\hat{\bm n}$、翻转顺序、残余矢量要求 & 当前 RF 菜单、纯张量可用性、Wien 标定 \\
源与加速接口 & 模式同步、有效区间、单圈质量记录 & 本次配方、调束与维护负责人、恢复程序 \\
极化仪运行 & 靶、符合 DAQ、束斑监测、分析重放 & 安装接口、可借用设备、强度与真空条件 \\
标定尺度 & 分析本领版本、坐标约定、误差传递 & 参考标定数据、原始记录与系统误差 \\
靶点输运 & 可验证旋转模型及两站产额比较 & 当前场图、磁铁设定、轨道、实际靶位 \\
束流期支持 & 在线极化、漂移诊断、数据有效性判断 & 值班责任、故障联络、重新验收条件 \\\bottomrule
\end{tabularx}
\end{table}

\clearpage
\subsection{用四项结果判断实验侧能力是否已经接住}
\begin{enumerate}
\item \textbf{逐模式参考态：}给出实际 $P,T$、轴方向及协方差，能回溯至测量时间、RF 菜单和源状态；明确纯张量或混合矢量模式的适用方式。
\item \textbf{独立极化分析：}从符合事件、归一化计数和版本化标定表重算极化，并能用源模式变化、非极化参考和分析选择稳定性检查结果。
\item \textbf{实际靶点预测与验证：}以本次磁铁与靶位为输入，解释参考站到下游站的响应，将最后一段输运延伸到反应顶点并传播不确定度。
\item \textbf{可执行运行规则：}知道哪些时间段数据有效、发生异常应检查什么、由谁处理设施问题，以及恢复后需重测哪些量。
\end{enumerate}
这四项共同把极化源提供的束流变成物理分析可用的靶点自旋状态。公开文献支持其方法和历史可行性；本次配置、可用模式和现场责任应体现在交接数据中。
...
研究组报告\cite{SekiguchiLab2025} & 1／370 & 研究方向、FY2024 活动、Members；本文图\ref{fig:sek-role}。\\
