...
> 状态：历史实验笔记，不是当前 CompactInVacuum SiPM 探测器或机械设计依据。
> 未入库的原始截图以缺失标记保留，结论性描述应视为待实验复核的假设。

*原始截图 1753452129490、1753452429094 和 1753452442226 未随仓库保存。*


这个波形有很多波折，初步假设是终端/源端阻抗匹配或采样率问题；需用已知 50 $\Omega$ 终端、电缆时延和更高带宽重复测量后才能定性。

*原始截图 1753963907599 未随仓库保存。*
...
以下几幅图为50 $\Omega$
*原始 50 $\Omega$ 截图 1754201471343 未随仓库保存。*


以下几幅图为 1M $\Omega$

*原始 1 M$\Omega$ 截图 1754201509818 未随仓库保存。*

![1 MΩ 波形](assets/5e92409e4199b404d43dd7d3e700360.jpg)

公司的测试视频

*原始视频截图 1754201626480 未随仓库保存。*


可以发现，公司测试时候示波器估计使用的是1M欧姆阻抗。 下降时间40微秒量级。 这个参数也不行。

*原始截图 1754204211169 未随仓库保存。*

![小信号反射波形](assets/c4b3c19b5ea35a9e9f86ae1277d31b9.jpg)

<font color="red">怀疑是base读出的问题，把幅度放小之后，这些小信号有很强的反射。 可能是在分压电路，或者滤波电路里有很强的反射。</font>

1 M$\Omega$ 波形可能包含信号在 PMT--传输线--示波器之间的多次反射叠加。对 $Z_0=50\,\Omega$、$Z_L=1\,\mathrm{M}\Omega$ 的负载端，$\Gamma_L = \frac{Z_L-Z_0}{Z_L+Z_0} \approx 0.99990$。实际波形还取决于 PMT 源阻抗、base/滤波网络和电缆往返时延，不能仅由这一估算确证。
