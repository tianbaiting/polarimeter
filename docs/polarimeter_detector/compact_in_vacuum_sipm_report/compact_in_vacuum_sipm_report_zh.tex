...
\usepackage[
  ...
  pdftitle={CompactInVacuum SiPM 探测器、真空靶室与中国采购工程报告}
]{hyperref}
\usepackage{bookmark}
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\sffamily\scriptsize COMPACTINVACUUM 极化仪}
...
\begin{titlepage}
  ...
  \begin{tabularx}{\textwidth}{>{\sffamily\bfseries}L{0.23\textwidth}Y}
    ...
    配置文件 & \path{compactInVacuum/config/default_compactInVacuum.yaml} \\
  ...
  \vfill
  \begin{warningbox}{初步报告 -- 不得直接用于加工或采购订单}
    本报告把当前物理分析和 CAD 状态整理为一个建议工程基线。所有开放尺寸、
    真空要求、接口图纸、材料证书和供应商能力必须在规定的开发门审中关闭。
  \end{warningbox}
...
\begin{warningbox}{已固定的 ICF114 合同与当前 CAD 通光孔冲突}
  项目规划两台极化仪。afterSRC 外置探测器版本的前、后束流接口固定为 ICF114，
  顶部旋转馈通接口固定为 ICF70。CompactInVacuum 的前、后束流接口也固定为
  ICF114，旋转馈通安装接口现也固定为 ICF70；具体旋转馈通器尚未选型。
  允许使用转接器和波纹管，但必须保持规定的全金属密封真空等级。

  当前 CAD 使用 \SI{63.6}{\milli\meter} 管外径和
  ...
  ICF114 不是开放选项。应依据已认证的 ICF114 法兰、接管、转接器和波纹管图纸
  冻结可用孔径，再修改内部管道和 stay-clear 几何。
\end{warningbox}
...
本项目包含两台不同用途的极化仪：
\begin{itemize}
  \item \textbf{afterSRC 极化仪：}探测器位于专用靶室外，沿用已经验证的
    H+V+V 机械布局。前端 ICF114、后端 ICF114、顶部 ICF70 旋转馈通，
    不增加侧面真空口。
  \item \textbf{CompactInVacuum 极化仪：}作为后续主要开发版本，
    探测器在真空内工作，前后端束流接口固定为 ICF114，顶部旋转接口固定为
    ICF70。当前概念基线另含四个按方位分组的 ICF70 四通道同轴接口，
    以及一个 ICF70 32 针 housekeeping 接口。
\end{itemize}

本报告主要讨论 CompactInVacuum 的探测器技术，同时把 afterSRC 接口合同
作为不可改变的机械约束。参考物理工况为
\SI{\BeamEnergyMeV}{\mega\electronvolt} 氘束轰击 CH$_2$ 靶，
在四个方位扇区中对弹性散射氘核与质子作 coincidence 探测。

\begin{table}[htbp]
  \centering
  \small
  \caption{两台极化仪的固定真空接口合同。精确孔径、螺栓几何和安装包络以供应商
  认证图纸为准。}
  \begin{tabularx}{\textwidth}{L{0.20\textwidth}L{0.24\textwidth}L{0.24\textwidth}Y}
    \toprule
    仪器 & 束流接口 & 其他真空接口 & 固定要求 \\
    \midrule
    afterSRC & 前 ICF114；后 ICF114 &
      顶部 ICF70 旋转馈通；无附加侧面真空口 &
      即使使用转接器或波纹管，外部配合面仍为 ICF114 \\
    CompactInVacuum & 前 ICF114；后 ICF114 &
      顶部 ICF70 旋转接口；四个 ICF70 四通道同轴接口；
      一个 ICF70 32 针 housekeeping 接口 &
      安装标准和容量已固定；供应商型号仍为接口包络 \\
    两台仪器 & 采购 ICF 刀口件和 OFHC 铜垫片 &
      靶室、焊缝、转接器、波纹管和馈通构成一个验收边界 &
      整机氦漏率 $\leq
      \IcfAssemblyLeakMantissa\times10^{-10}\,
      \si{\pascal\meter\cubed\per\second}$ \\
    \bottomrule
  \end{tabularx}
  \datatag{docs/specs/BLP_v1_requirement_baseline.md；
  compactInVacuum/config/default_compactInVacuum.yaml}
\end{table}

\subsection{当前探测器与靶室包络}
...
当前方形靶室内截面约为 $440\times440$ mm，轴向长度约
...
\begin{itemize}
  ...
  \item 不把十二路快速信号馈通分散布置在曲面壳体上，除非端口布局和维护评审
    证明这种方案有明确优势。
  \item 束流两端使用短、可拆 ICF 接管，运输时安装刀口保护盖，并设置吊点和
    可人工搬运子组件质量上限。
\end{itemize}

\section{探测器物理与有效厚度}

现有能损分析最初服务于旧外置探测器布局。本报告按当前 CompactInVacuum
...
推荐每个探测器使用一条屏蔽 \SI{50}{\ohm} 同轴通道。
信号和偏置通过外部 bias tee 共用同轴线，有源模拟电子学保持在真空外。
十二个探测器需要十二条可独立测试的同轴馈通通道，另设低速温度与 housekeeping
接口。当前 CAD 按方位分组：四个 ICF70 四通道同轴接口在每个方位使用三路、
保留一路安装备用；另一个 ICF70 32 针接口为 12 个两线 SiPM 温度传感器使用
24 针并保留 8 针。准确采购料号仍未冻结。
...
\begin{itemize}
  ...
  \item 两端使用认证 ICF114 束流接管；
  \item 两台仪器均设置专用 ICF70 旋转馈通凸台。CompactInVacuum 已固定
    安装接口，但具体采购件、轴载荷、扭矩、回差、寿命和烘烤温度仍待 RFQ 关闭。
\end{itemize}

\subsection{固定 ICF 接口合同}

采购经认证的 ICF 法兰或半接管。上海谱幂可以完成焊接和集成，
...
孔径、螺栓和配合合同必须由供应商图纸定义。

允许使用转接器和波纹管解决对准、振动隔离和安装公差问题。
每个采购件必须提供签字图纸、材料和清洗记录、烘烤等级与氦漏证书。
完成焊接和安装后的整机还必须再次检漏；单个部件证书不能替代系统验收。

\subsection{旋转馈通 RFQ 数据}

两台仪器目前都固定使用顶部 ICF70 旋转馈通安装接口。当前机构研究采用约
\SI{18}{\milli\meter} 轴和 0/90 度工作位置。对 CompactInVacuum 而言，
固定 ICF70 并不等于已经选定供应商机构；载荷与服务研究关闭前仍使用接口包络。
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{L{0.18\textwidth}Y Y}
    ...
    材料 & 304L 或 316L 不锈钢，炉号/批次可追溯；最终牌号由真空和磁性评审决定 &
      材料证书与追溯记录 \\
    采购 ICF 件 & 使用认证法兰/半接管，不逆向仿制刀口 &
      供应商图纸、证书和进货检验 \\
    焊接 & 完整焊缝图、工艺、焊工资格、顺序和变形控制 &
      焊接计划、WPS/PQR、焊接记录和外观记录 \\
    真空设计 & ICF 刀口/OFHC 主密封；无积气体积；盲孔通气；内部可清洗；
      转接器和波纹管不得引入弹性体降级 &
      剖面图、密封清单和设计评审表 \\
    结构 & 外压屈曲、搬运载荷和接口载荷 &
      计算报告和批准图纸 \\
    ...
    清洗 & 与探测器和真空材料相容的受控工艺 &
      清洗、包装和封口记录 \\
    烘烤 & 裸金属靶室与采购 ICF 硬件的等级和装有探测器后的系统温限分开定义 &
      部件温限和批准的整机最高温度 \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{L{0.17\textwidth}L{0.25\textwidth}L{0.25\textwidth}Y}
    ...
    Gate 2：靶室首件 & 一套靶室、一套旋转馈通、ICF 件 &
      一套完整首件 & DFM、尺寸、外压、检漏、烘烤和维护测试 \\
    量产 & 十二个工作通道加备件 &
...
\begin{enumerate}
  \item \textbf{Gate 0 -- 冻结物理和接口。}
    固定 ICF114 前后接口和 afterSRC 顶部 ICF70；批准束流 stay-clear、
    探测器半径/直径、5--6 mm 厚度研究带、靶运动包络、装机烘烤温限、
    端口图、测量基准和运输限制。
  ...
  \item \textbf{Gate 4 -- 工程靶室。}
    完成上海谱幂首件、外压复核、CMM 报告、氦检、靶运动、探测器拆装和运输演示。
  \item \textbf{Gate 5 -- 量产放行。}
    只有 golden sector 与工程靶室通过后，才能冻结图纸和流转卡并采购量产通道。
\end{enumerate}
...
\begin{longtable}{L{0.27\textwidth}C{0.10\textwidth}C{0.10\textwidth}L{0.37\textwidth}}
  ...
  \midrule
  \endhead
  当前 \SI{63}{\milli\meter} stay-clear 与参考 ICF114 净孔不相容 &
    高 & 高 & 保持 ICF114 固定；按认证接管/转接器/波纹管孔径替换占位几何，
    重新分配内部通光孔预算 \\
...
任何生产采购前必须完成：
\begin{itemize}
  \item 对照认证 ICF114 接管、转接器和波纹管图纸批准 stay-clear；
    ICF114 已固定，不再比较法兰尺寸；
  \item 批准 0.8 比例几何，或明确保留当前半径和
    \SI{25}{\milli\meter} 接受度；
  \item 批准 Gate 1 的 5 mm、6 mm 探测盒和比较样片；
  \item 批准 ICF 全金属边界与整机漏率，再定义工作压力、气负载、
...
\begin{longtable}{L{0.19\textwidth}L{0.34\textwidth}L{0.38\textwidth}}
  ...
  真空边界 & 工作压力、清洗/烘烤要求、固定 ICF 接口 &
    材料/焊接、通气设计、外压计算、检漏方法和证书 \\
  束流接口 & 固定 ICF114 前后接口、stay-clear 和配合图纸 &
    采购件身份、刀口保护、法兰位置/方向和检验方法 \\
...
该命令执行：
\begin{enumerate}
  \item 读取当前 CompactInVacuum YAML、target、通道清单和 strict validation 报告；
...
\begin{notebox}{配置状态警告}
  当前 CAD 源仍写着 active medium: placeholder/undecided 和
