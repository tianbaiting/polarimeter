...
$440\times440$ mm、轴向长度 \SI{360}{\milli\meter}、壁厚
\SI{8}{\milli\meter}。pre-SAMURAI 独立采用 $450\times450$ mm、
\SI{380}{\milli\meter} 长、暂定 \SI{8}{\milli\meter} 壁厚的方形方案。
afterSRC 配置不再保留圆柱形候选。旧 \SI{10}{\milli\meter} 壁厚封闭方壳材料体积筛算为
\SI{\ChamberMaterialVolumeL}{\liter}，按不锈钢密度估算约
\SI{\ChamberScreeningMassKg}{\kilogram}；该旧数值只保留作质量算术比较，
不是当前 afterSRC 壁厚模型，也不能代替外压屈曲、开孔、焊缝、支撑和搬运 FEA。
...
\begin{itemize}
  ...
  \item 顶部采用可拆服务歧管，并利用平面靶室壁布置测量基准和四组信号接口，
    使扇区径向抽出、工具空间、线缆路径和焊接变形能够直接评审。
...
平面壁便于安装服务歧管、建立测量基准、提供工具空间，并保证扇区径向拆卸。
CompactInVacuum-afterSRC 不保留圆柱形靶室方案。

选定架构包括：
\begin{itemize}
  \item 针对实际开孔、焊缝、支撑和搬运载荷验证的方形外压靶室；
  \item 集中信号馈通和线缆路径的可拆顶部服务歧管；
  \item 具有径向抽出与工具空间的四套确定性扇区载架接口；
...
\begin{itemize}
  ...
  \item 批准 Gate 1 的 5 mm、6 mm 短探测器头和比较样片；
