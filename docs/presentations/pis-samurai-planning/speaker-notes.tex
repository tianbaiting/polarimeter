...
\newcommand{\noteprecisiontwenty}{%
这张图保持上一页全部束流、靶和几何条件，只把时间从 10 s 改为 20 s。两幅图采用相同坐标范围和相同的 68.3\%、95\% 色带定义，可以直接比较区间宽度。

当真实 $p_{yy}=1$，20 s 内预计有 \PyyTwentyAtOneCount{} 个有效符合计数。名义 68.3\% 区间缩窄到 $[\PyyTwentyAtOneLowA,\PyyTwentyAtOneHighA]$，95\% 区间为 $[\PyyTwentyAtOneLowB,\PyyTwentyAtOneHighB]$。

当真实值为 0.8，预计计数为 \PyyTwentyAtEightCount{}，68.3\% 区间为 $[\PyyTwentyAtEightLowA,\PyyTwentyAtEightHighA]$，95\% 区间为 $[\PyyTwentyAtEightLowB,\PyyTwentyAtEightHighB]$。虚线上的最佳拟合值接近真实值，是因为输入采用期望计数，并非说明真实测量不会有统计涨落。

远离物理边界时，时间加倍使统计误差大致缩小为 $1/\sqrt2$，即约缩小 29\%。靠近 $p_{yy}=1$ 时，置信区间不再对称。两种时间下的区间上端都可达到 1，因为拟合限定在物理域内；不能把这一平坦上边界解释成上侧测量误差为零。

这些曲线给出当前模型和暂定计数条件下的比较。实际效率、活时间、靶厚或束流强度降低，都会减少有效计数并扩大区间；背景、分析本领及相对扇区效率的不确定度还会带来额外误差。因此会上可据此讨论 10 s 或 20 s 的监测周期是否足够，并确认这些输入。
...
高束流可以缩短同样计数所需的时间，但极化误差还取决于分析本领、角度覆盖、背景和归一化。前两页曲线已用当前配置估计 10 s 和 20 s 的典型数据置信区间；这些是附带假设的设计估计。还需确认实际束流强度、靶厚、效率与所需 $p_{yy}$ 精度，才能确定正式监测周期。
