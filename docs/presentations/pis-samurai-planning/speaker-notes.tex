...
\newcommand{\noteone}{%
我们这次希望确定的是：RIKEN 能否稳定提供竖直方向的纯 tensor 氘束，以及如何监测它的极化。这里以实验室竖直方向为 $y$ 轴，先只讨论理想 $(p_y,p_{yy})=(0,+1)$ 的模式。实际束流要求 $p_y$ 接近零，$p_{yy}$ 的幅度需测量确认。

对自旋为 1 的氘核，沿 $y$ 轴有三个磁子态。正 tensor 理想态把粒子等量分到 $m_y=+1$ 和 $-1$，$m_y=0$ 的布居为零，给出 $p_{yy}=+1$，vector polarization 为零。实际源的极化幅度和残余 vector 分量都要测量。建议同时讨论无极化参考模式及切换周期。
...
按照 1995 年 RIKEN 报告的能级编号，理想 $(p_y,p_{yy})=(0,+1)$ 模式关闭 SFT\#1，WFT\#1 做 $1\to4$，WFT\#2 关闭。此时第二组六极磁铁出口保留 2、3 态。SFT\#2 使用 $2\to6$，最终为 3、6 态。不能把这些超精细态编号直接当成核自旋磁量子数。
% [EN] Defer the negative-tensor transition and final states. / [CN] 暂不展示负张量模式的跃迁和最终态。
% 负 tensor 使用 SFT\#2 的 $3\to5$，最终为 2、5 态。

这份历史配方没有给出适用于当前装置的频率、磁场和功率设定。需要 RIKEN 团队确认当前 recipe 和跃迁效率，并测量该模式的 $p_y$ 和 $p_{yy}$。
% [EN] Defer positive/negative tensor switching. / [CN] 暂不讨论正负张量模式切换。
% 原表脚注明确说，在 RF 频率固定时改变 SFT\#2 磁场，可以切换 $2\to6$ 和 $3\to5$。

原表曾测得这一正 tensor 模式的 tensor 分量约为 $+0.77$。这是历史性能，不是本次实验的交付指标。我们也不根据新实验中的混合极化模式，推定当前纯 tensor 模式的性能。
% [EN] Defer the historical negative-mode measurement. / [CN] 暂不展示负模式的历史测量值。
% 负 tensor 模式的实测 tensor 分量约为 $-1.23$。

\textbf{建议口头问题：}Is this positive pure-tensor RF mode available in the current PIS configuration, and what residual vector polarization should we expect?
