...
\ifdefined\SpeakerNotes
    ...
    \notepage{7}{20 秒测量的预期置信区间}{\noteprecisiontwenty}\clearpage
...
\begin{frame}[t]{Current polarimeter: expected precision in 20 s}
\centering
\includegraphics[width=\textwidth,height=104mm,keepaspectratio]{pyy_confidence_20s.pdf}\par
\vspace{2mm}
{\smalltext\color{Teal}\bfseries At true $p_{yy}=1$: 68.3\% CI $[\PyyTwentyAtOneLowA,\PyyTwentyAtOneHighA]$,\quad
95\% CI $[\PyyTwentyAtOneLowB,\PyyTwentyAtOneHighB]$\par}
\source{Same geometry, beam, and provisional target as the 10 s plot. Doubling the exposure doubles expected counts. Both plots use identical axis limits.\\
