...
\ifdefined\SpeakerNotes
    ...
    \notepage{6}{10 秒测量的预期置信区间}{\noteprecisionten}\clearpage
    \notepage{7}{60 秒测量的预期置信区间}{\noteprecisionsixty}\clearpage
    \notepage{8}{希望这次会议确定的事项}{\notesix}
\else
...
% [EN] Asimov intervals describe typical-data confidence limits, not the distribution of repeated estimates. / [CN] 典型数据置信区间描述参数限值，不是重复测量估计值的分布。
\begin{frame}[t]{Current polarimeter: expected precision in 10 s}
\centering
\includegraphics[width=\textwidth,height=104mm,keepaspectratio]{pyy_confidence_10s.pdf}\par
\vspace{2mm}
{\smalltext\color{Teal}\bfseries At true $p_{yy}=1$: 68.3\% CI $[\PyyTenAtOneLowA,\PyyTenAtOneHighA]$,\quad
95\% CI $[\PyyTenAtOneLowB,\PyyTenAtOneHighB]$\par}
\source{Current 20 mm heads, forward $p$--$d$ coincidences: $p$ at $53.4^\circ$/205 mm and $d$ at $20.9^\circ$/140 mm, four sectors. The model uses a $20\times20$ mm acceptance approximation.\\
Source: current\_tensor.ini and dpolar Asimov profile likelihood. Fixed analyzing powers, unit efficiency/live time, no background or target energy-loss correction. Confidence levels are nominal, especially at the $p_{yy}=1$ boundary.}
\note{\noteprecisionten}
\end{frame}

\begin{frame}[t]{Current polarimeter: expected precision in 60 s}
\centering
\includegraphics[width=\textwidth,height=104mm,keepaspectratio]{pyy_confidence_60s.pdf}\par
\vspace{2mm}
{\smalltext\color{Teal}\bfseries At true $p_{yy}=1$: 68.3\% CI $[\PyySixtyAtOneLowA,\PyySixtyAtOneHighA]$,\quad
95\% CI $[\PyySixtyAtOneLowB,\PyySixtyAtOneHighB]$\par}
\source{Same geometry, beam, and provisional target as the 10 s plot. Six times the exposure gives six times the expected counts. Both plots use identical axis limits.\\
Source: current\_tensor.ini and dpolar Asimov profile likelihood. Statistical only, fixed relative sector response. The upper edge at 1 follows the physical parameter bound.}
\note{\noteprecisionsixty}
\end{frame}
