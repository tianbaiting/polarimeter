...
\lead{Requested beam: $p_y\simeq0$, $p_{yy}\neq0$, with the symmetry axis along vertical $y$}
...
\lead{Historical RIKEN scheme: change the second strong-field transition}
\smalltext
...
\vspace{2mm}
Final states: $3,6$ for $+1$, or $2,5$ for $-2$.
\column{0.47\textwidth}
\textbf{Switching method in the report}\par
\vspace{2mm}
...
\begin{frame}[t]{PIS operation and local support}
\begin{columns}[T,onlytextwidth]
...
\column{0.48\textwidth}
\lead{Support to arrange for our run}
If Prof. Sekiguchi is unavailable on site, who at RIKEN can operate the PIS?\par
\vspace{4mm}
\smalltext
Who handles startup and RF-mode tuning?\par
\vspace{3mm}
Who checks polarization during the run?\par
\vspace{3mm}
What preparation time or training is needed?\par
\vspace{5mm}
{\color{Muted}N. Sakamoto is among the RIKEN collaborators named in the reports. Current operating responsibility requires confirmation.}
\end{columns}
...
\midrule
PIS support & Local operator and preparation time. Support when Prof. Sekiguchi is unavailable.\\[2mm]
