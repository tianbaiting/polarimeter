% !TEX program = xelatex
% [EN] The notes and slides share the same physics and source definitions. / [CN] 讲稿与幻灯片共用物理内容和来源定义。
\ifdefined\SpeakerNotes
  \documentclass[12pt,a4paper]{article}
  \usepackage[margin=24mm]{geometry}
  \usepackage{hyperref}
\else
\documentclass[aspectratio=169,14pt]{beamer}
  \geometry{paperwidth=320mm,paperheight=180mm}
\fi
\usepackage{lmodern,fontspec,xeCJK}
\setmainfont{TeX Gyre Heros}
\setsansfont{TeX Gyre Heros}
\setCJKmainfont{Noto Sans CJK SC}
\setCJKsansfont{Noto Sans CJK SC}
\setCJKmonofont{Noto Sans Mono CJK SC}
\usepackage{amsmath,amssymb,booktabs,tabularx,array,graphicx,xcolor,xurl}
\definecolor{Navy}{HTML}{16324D}
\definecolor{Teal}{HTML}{007F86}
\definecolor{Ink}{HTML}{26323B}
\definecolor{Muted}{HTML}{586974}
\definecolor{Rust}{HTML}{A14D22}
\hypersetup{colorlinks=true,urlcolor=Teal,linkcolor=Teal,
 pdftitle={Polarized deuteron beam preparation for SAMURAI},pdfauthor={Baiting Tian}}
\newcolumntype{Y}{>{\raggedright\arraybackslash}X}
\newcommand{\src}[2]{\href{#1}{#2}}
\newcommand{\sakamoto}{https://proceedings.jacow.org/cyclotrons2016/papers/tub04.pdf}
\newcommand{\hyperfineref}{https://nucldata.tunl.duke.edu/nucldata/Theses/PhD/Sabourov_2006.pdf}
\newcommand{\rfprinciple}{https://arxiv.org/pdf/1212.1840}
\newcommand{\okamura}{https://www.nishina.riken.jp/researcher/APR/Document/ProgressReport_vol_28.pdf}
\newcommand{\sugahara}{https://indico.cern.ch/event/1441702/contributions/7106564/}
\newcommand{\avftest}{https://indico.ibs.re.kr/event/701/contributions/6590/}
\newcommand{\fthreelimit}{https://www.nishina.riken.jp/ribf/BigRIPS/intensity.html}
\newcommand{\bis}{https://www.nishina.riken.jp/researcher/APR/APR058/pdf/98.pdf}
\newcommand{\attlocations}{https://www.nishina.riken.jp/researcher/APR/APR047/pdf/156.pdf}
\newcommand{\slitsource}{https://www.nishina.riken.jp/researcher/APR/APR052/pdf/130.pdf}
\newcommand{\attguide}{https://ribf.riken.jp/sisetu-kyoyo/HIbeam/BmL/BmL\%E4\%BD\%9C\%E6\%96\%B9111__\%E7\%90\%86\%E7\%A0\%94\%E3\%81\%AE\%E5\%A0\%B4\%E5\%90\%88.pdf}
\newcommand{\deckpages}{9}
\input{build/pyy-confidence/precision-values.tex}
\newcommand{\precisionrow}[2]{#1 & \csname Pyy#2Count\endcsname &
 $[\csname Pyy#2LowA\endcsname,\csname Pyy#2HighA\endcsname]$ &
 $[\csname Pyy#2LowB\endcsname,\csname Pyy#2HighB\endcsname]$\\}
\newcommand{\precisiontable}[1]{%
\par\noindent
\begin{tabularx}{\linewidth}{@{}Y*{3}{>{\centering\arraybackslash}X}@{}}
\toprule
Exposure & Expected counts & 68.3\% CI & 95\% CI\\
\midrule
\precisionrow{10 s}{#1TenAtOne}
\precisionrow{60 s}{#1SixtyAtOne}
\bottomrule
\end{tabularx}\par}
\input{speaker-notes.tex}

\ifdefined\SpeakerNotes
  \setlength{\parindent}{0pt}
  \setlength{\parskip}{0.7em}
  \setlength{\emergencystretch}{2em}
  \newcommand{\notepage}[3]{
    {\color{Muted}\small SAMURAI meeting \quad 中文讲稿 \quad #1/\deckpages\par}
    \vspace{0.6em}{\color{Navy}\LARGE\bfseries #2\par}\vspace{0.7em}
    #3
  }
\else
\setbeamercolor{normal text}{fg=Ink,bg=white}
\usefonttheme{professionalfonts}
\setbeamercolor{frametitle}{fg=Navy,bg=white}
\setbeamercolor{structure}{fg=Teal}
\setbeamercolor{alerted text}{fg=Rust}
\setbeamerfont{normal text}{size=\fontsize{20}{25}\selectfont}
\setbeamerfont{frametitle}{size=\fontsize{32}{37}\selectfont,series=\bfseries}
\setbeamersize{text margin left=13mm,text margin right=13mm}
\setbeamertemplate{navigation symbols}{}
\setbeamertemplate{frametitle}{\vspace{5mm}\insertframetitle\par\vspace{3mm}}
\setbeamertemplate{footline}{\hspace{13mm}{\color{Muted}\fontsize{10}{12}\selectfont SAMURAI beam preparation\hfill\insertframenumber/\deckpages\hspace{13mm}}\vspace{4mm}}
\newcommand{\smalltext}{\fontsize{17}{22}\selectfont}
\newcommand{\source}[1]{\vfill{\color{Muted}\fontsize{10}{13}\selectfont #1\par}}
\newcommand{\lead}[1]{{\color{Teal}\bfseries #1\par}\vspace{3mm}}
\renewcommand{\arraystretch}{1.35}
\title{Polarized deuteron beam preparation for SAMURAI}
\author{Baiting Tian}
\date{11 September 2026}
\fi
\begin{document}
\ifdefined\SpeakerNotes
    \notepage{1}{氘原子的六个超精细态}{\noteone}\clearpage
    \notepage{2}{PIS 如何用 RF 控制核自旋布居}{\notetwo}\clearpage
    \notepage{3}{RIKEN 能否独立完成极化氘束调试}{\notethree}\clearpage
    \notepage{4}{三个测偏位置的比较}{\notefour}\clearpage
    \notepage{5}{F3 限制、ATT 与狭缝}{\notefive}\clearpage
    \notepage{6}{基准强度：10 秒与 60 秒}{\noteprecisionreference}\clearpage
    \notepage{7}{ATT / slits 透过率 1/10：计数乘 10}{\noteprecisiongainTen}\clearpage
    \notepage{8}{ATT / slits 透过率 1/100：计数乘 100}{\noteprecisiongainHundred}\clearpage
    \notepage{9}{希望这次会议确定的事项}{\notesix}
\else
\usebeamerfont{normal text}

\begin{frame}[t]{Six hyperfine states in a deuterium atom}
\lead{Before ionization, the ground-state atom contains an electron and a spin-1 nucleus}
\begin{columns}[T,onlytextwidth]
\column{0.50\textwidth}
\[
 S=\tfrac12,\quad I=1
 \quad\Longrightarrow\quad (2S+1)(2I+1)=6
\]
\smalltext
At $B=0$, electron and nuclear spins couple:\par
\vspace{1mm}
$F=3/2$: four substates.\quad $F=1/2$: two substates.\par
\vspace{5mm}
\textbf{State labels in the strong-field limit}\par
\vspace{1mm}
\begin{tabularx}{\linewidth}{@{}p{37mm}*{3}{>{\centering\arraybackslash}X}@{}}
\toprule
 & $m_I=+1$ & $m_I=0$ & $m_I=-1$\\
\midrule
$m_S=+1/2$ & $|1\rangle$ & $|2\rangle$ & $|3\rangle$\\
$m_S=-1/2$ & $|6\rangle$ & $|5\rangle$ & $|4\rangle$\\
\bottomrule
\end{tabularx}\par
\vspace{4mm}
At weaker fields, the eigenstates generally mix electron and nuclear spin projections.\par
\column{0.46\textwidth}
% [EN] Original Fig. 2.3, PDF page 47, cropped by Poppler at scale 2000: x=480, y=375, width=690, height=575. / [CN] 原图 2.3，PDF 第 47 页，以 Poppler 按上述像素框裁取，未改动图内内容。
\includegraphics[width=\linewidth,height=83mm,keepaspectratio]{figures/deuterium-hyperfine-states.png}\par
\vspace{2mm}
\smalltext
The static field $B_0$ sets the six energies\par
\vspace{1mm}
$E_1(B_0),E_2(B_0),\ldots,E_6(B_0)$.\par
\vspace{2mm}
Their separations determine the RF resonances.
\end{columns}
\source{Source and original Breit--Rabi diagram: \src{\hyperfineref}{Sabourov, Duke PhD thesis (2006), pp. 22--23, Fig. 2.3 / PDF pp. 46--47}. For the $1s$ electron, $J=S$, so the figure's $m_j$ is the table's $m_S$. Spin projections refer to the local static-field axis.}
\note{\noteone}
\end{frame}

\begin{frame}[t]{RF transitions control the nuclear spin populations}
\smalltext
\setlength{\abovedisplayskip}{3pt}
\setlength{\belowdisplayskip}{6pt}
\vspace{-2mm}
\[
 \mathbf B_{\rm RF}(t)=\mathbf B_1\cos(2\pi\nu t),\qquad
 h\nu=\left|E_j(B_0)-E_i(B_0)\right|
 \quad\text{for an allowed single-photon transition}
\]
\vspace{-2mm}
\begin{columns}[T,onlytextwidth]
\column{0.48\textwidth}
\textbf{WFT = Weak Field Transition}\par
Weak static field: within an $F$ multiplet.
\column{0.48\textwidth}
\textbf{SFT = Strong Field Transition}\par
Transfers between the two hyperfine multiplets.
\end{columns}
\vspace{3mm}
\lead{RF redistributes populations. Sextupoles select which atoms remain in the beam.}
\renewcommand{\arraystretch}{1.12}
\begin{tabularx}{\textwidth}{@{}p{43mm}p{37mm}Y@{}}
\toprule
Stage & States & Role in the ideal $p_{yy}=+1$ recipe\\
\midrule
Sextupole \#1 & $1,2,3$ & Focus the selected electron-spin group.\\
WFT\#1 & $2,3,4$ & Transfer $1\to4$ effectively through intermediate states.\\
Sextupole \#2 & $2,3$ & Reject state 4 and retain the focused states.\\
SFT\#2 & $3,6$ & Transfer $2\to6$, filling the $m_I=+1$ branch.\\
\bottomrule
\end{tabularx}\par
\vspace{2mm}
ECR removes the electron. Equal populations of states 3 and 6 give, ideally:\par
\vspace{1mm}
\( (n_+,n_0,n_-)=(\tfrac12,0,\tfrac12)\quad\Longrightarrow\quad
 p_y=n_+-n_-=0,\qquad p_{yy}=1-3n_0=+1.\)\par
\vspace{2mm}
{\fontsize{13}{16}\selectfont The nuclear populations use the strong-field limit and ideal polarization retention. SFT\#1 and WFT\#2 are off. We retain the vertical source axis without additional Wien-filter rotation.}\par
\source{Sources: \src{\rfprinciple}{Mikirtychyants et al., arXiv:1212.1840, Sec. II G}; \src{\okamura}{Okamura et al., RIKEN APR 28 (1995), Table 1, p. 148}; \src{\sakamoto}{Sakamoto et al., 2016, p. 146}. Historical recipe with ideal state selection, transfer, and ionization.}
\note{\notetwo}
\end{frame}

% [EN] Defer the negative-tensor population row. / [CN] 暂不展示负张量模式的布居。
% $p_{yy}=-2$ & $0$ & $1$ & $0$\\
% [EN] Defer the negative-tensor RF recipe. / [CN] 暂不展示负张量模式的 RF 配方。
% $(0,-2)$ & Off & $1\to4$ & $3\to5$ & Off\\
% [EN] Deferred negative-mode final states. / [CN] 暂不展示负模式的最终态。
% Final states: $2,5$ for $p_{yy}=-2$.
% [EN] Defer switching between positive and negative tensor modes. / [CN] 暂不讨论正负张量模式之间的切换。
% Keep the RF frequency fixed and change the SFT\#2 magnetic field.

\begin{frame}[t]{Independent polarized-deuteron tuning at RIKEN}
\begin{columns}[T,onlytextwidth]
\column{0.48\textwidth}
\lead{Recent operating evidence}
\textbf{September 2024}\par
\vspace{2mm}
AVF test at 7 MeV/u after PIS repair.\par
\vspace{4mm}
\textbf{January 2026}\par
\vspace{2mm}
Polarized-beam experiment at 100 MeV/u:\par
\[
\begin{aligned}
 p_y&=-0.615\pm0.005,\\
 p_{yy}&=+0.896\pm0.012.
\end{aligned}
\]
\smalltext This operating mode contains vector polarization.
\column{0.48\textwidth}
\lead{Capability to confirm for our run}
Can RIKEN staff tune the polarized deuteron beam without assistance from Prof. Sekiguchi?\par
\vspace{4mm}
\smalltext
Who can set up PIS and this pure-tensor RF mode?\par
\vspace{3mm}
Who can tune acceleration and beam transport?\par
\vspace{3mm}
Who can measure and verify the polarization?\par
\vspace{5mm}
{\color{Muted}The reports name RIKEN collaborators including N. Sakamoto. Independent tuning capability and the responsible team require confirmation.}
\end{columns}
\source{Sources: \src{https://www.nishina.riken.jp/researcher/APR/APR058/pdf/36.pdf}{Sugahara et al., RIKEN APR 58 (2025), p. 36}; \src{\sugahara}{Sugahara et al., EFB 2026 contribution, January 2026 run}.}
\note{\notethree}
\end{frame}

\begin{frame}[t]{Three locations for beam polarimetry}
\begin{center}
\includegraphics[width=0.74\textwidth,height=48mm,keepaspectratio]{../../physics/polarization_production_transport/figures/sakamoto2016_p1_fig1.png}\par
{\color{Muted}\fontsize{11}{14}\selectfont Historical accelerator layout showing DPOL before SRC and BigDPOL after SRC}
\end{center}
\vspace{1mm}
\smalltext
\begin{tabularx}{\textwidth}{@{}p{38mm}p{71mm}Y@{}}
\toprule
Location & What it measures & Questions for use\\
\midrule
After AVF & Source modes & Calibration at AVF energy. Dedicated measurement or inline monitoring?\\
After RRC & Before SRC & DPOL access. Calibration and target effects on SRC injection.\\
After SRC & Final beam energy & Space, count rate, and permitted attenuation before F3.\\
\bottomrule
\end{tabularx}\par
\vspace{3mm}
\textbf{First enquiry: existing DPOL after RRC.}\quad After-SRC installation remains an option.
\source{Sources: \src{\sakamoto}{Sakamoto et al., 2016, Fig. 1}; \src{\avftest}{Sugahara et al., INPC 2025, AVF polarimeter and Faraday-cup test}. Relating any upstream measurement to the SAMURAI target requires a transport check.}
\note{\notefour}
\end{frame}

\begin{frame}[t]{Beam attenuation before F3}
\lead{F3 total beam rate: $\dot N_{\mathrm{F3}}\leq10^7$ particles/s}
{\smalltext Proposed order, subject to beamline and interlock approval:\par}
\vspace{0mm}
\[
\mathrm{SRC}\ \longrightarrow\ \mathrm{polarimeter}\ \longrightarrow\ \mathrm{attenuation}\ \longrightarrow\ \mathrm{F3}\ \longrightarrow\ \mathrm{SAMURAI}
\]
\vspace{1mm}
\begin{columns}[T,onlytextwidth]
\column{0.48\textwidth}
\textbf{ATT: mesh beam attenuator}\par
\vspace{2mm}
\smalltext
Open apertures transmit part of the beam.\par
\vspace{2mm}
Nominal ratios: $1/2$, $1/3$, $1/5$, $1/10$, $1/100$.\par
\vspace{2mm}
Documented locations: C22, S31a and S64, upstream of RRC.\par
\vspace{2mm}
Upstream attenuation also lowers the after-SRC count rate.
\column{0.48\textwidth}
\textbf{Slits: acceptance restriction}\par
\vspace{2mm}
\smalltext
Select position, angle, or momentum.\par
\vspace{2mm}
F2--F3 angular-selection slits are documented.\par
\vspace{2mm}
Reduction depends on the beam distribution and aperture settings.\par
\vspace{2mm}
Assess stability, intercepted beam, and the polarization of the transmitted fraction.
\end{columns}
\vspace{2mm}
{\smalltext\color{Rust}\textbf{Interlock condition:} the 2025 BIS report describes an upstream-ATT check for primary-beam delivery. Downstream slits alone cannot be assumed to satisfy it.}
\source{Sources: \src{\fthreelimit}{BigRIPS F3 instructions}; \src{\attguide}{RIKEN intensity guide, p. 7}; \src{\attlocations}{APR 47 (2014), p. 156}; \src{\slitsource}{APR 52 (2019), p. 130}; \src{\bis}{APR 58 (2025), pp. 98--99}. Confirm the applicable operating mode and current interlock configuration.}
\note{\notefive}
\end{frame}

% [EN] Each page compares exposure at one count normalization. / [CN] 每页在同一计数归一化下比较两种测量时间。
\begin{frame}[t]{Reference intensity: 10 s and 60 s}
\begin{columns}[T,onlytextwidth]
\column{0.49\textwidth}
\includegraphics[width=\linewidth]{pyy_confidence_10s.pdf}
\column{0.49\textwidth}
\includegraphics[width=\linewidth]{pyy_confidence_60s.pdf}
\end{columns}
\vspace{1mm}
{\smalltext\color{Teal}\bfseries At true $p_{yy}=1$\par}
{\smalltext\renewcommand{\arraystretch}{1.05}\precisiontable{}}
\source{Current design, forward $p$--$d$ coincidences. 190 MeV/u, $I_{\rm pol}\simeq10^7$ d/s, provisional CH$_2$ 1000 mg/cm$^2$; 20 mm heads with existing $20\times20$ mm acceptance approximation.\\
Asimov profile likelihood, fixed analyzing powers and relative response, unit efficiency/live time, no background or energy-loss correction. Source: current\_tensor.ini and dpolar.}
\note{\noteprecisionreference}
\end{frame}

\begin{frame}[t]{ATT / slits: 1/10 transmission, counts $\times10$}
\begin{columns}[T,onlytextwidth]
\column{0.49\textwidth}
\includegraphics[width=\linewidth]{pyy_confidence_x10_10s.pdf}
\column{0.49\textwidth}
\includegraphics[width=\linewidth]{pyy_confidence_x10_60s.pdf}
\end{columns}
\vspace{1mm}
{\smalltext\color{Teal}\bfseries At true $p_{yy}=1$\par}
{\smalltext\renewcommand{\arraystretch}{1.05}\precisiontable{GainTen}}
\source{Hypothesis: $I_{\rm pol}\simeq10^8$ d/s at the polarimeter, followed by ATT/slits with transmission 1/10 before F3. F3 remains near $10^7$ d/s.\\
Multiply both LR and UD expected counts by 10 and recalculate the same profile intervals. Geometry, analyzing powers, efficiency, and all other assumptions match the reference.}
\note{\noteprecisiongainTen}
\end{frame}

\begin{frame}[t]{ATT / slits: 1/100 transmission, counts $\times100$}
\begin{columns}[T,onlytextwidth]
\column{0.49\textwidth}
\includegraphics[width=\linewidth]{pyy_confidence_x100_10s.pdf}
\column{0.49\textwidth}
\includegraphics[width=\linewidth]{pyy_confidence_x100_60s.pdf}
\end{columns}
\vspace{1mm}
{\smalltext\color{Teal}\bfseries At true $p_{yy}=1$\par}
{\smalltext\renewcommand{\arraystretch}{1.05}\precisiontable{GainHundred}}
\source{Hypothesis: $I_{\rm pol}\simeq10^9$ d/s at the polarimeter, followed by ATT/slits with transmission 1/100 before F3. F3 remains near $10^7$ d/s.\\
Multiply both LR and UD expected counts by 100 and recalculate the same profile intervals. All six plots share axis limits. Confidence levels are nominal, including at the physical boundary.}
\note{\noteprecisiongainHundred}
\end{frame}

\begin{frame}[t]{Decisions for beam preparation}
\smalltext
\renewcommand{\arraystretch}{1.2}
\begin{tabularx}{\textwidth}{@{}p{48mm}Y@{}}
\toprule
Topic & Agreement needed from the meeting\\
\midrule
RIKEN capability & Independent tuning without Prof. Sekiguchi's help. Responsible RIKEN team and preparation time.\\[2mm]
Polarization mode & Pure tensor RF recipe, residual $p_y$ tolerance, and vertical transport without additional Wien rotation.\\[2mm]
Polarimeter & Existing AVF/RRC device or after-SRC installation. Monitoring interval and target-side cross-check.\\[2mm]
Beam intensity & Attenuation location, ATT/slit settings, and applicable F3 interlock conditions.\\
\bottomrule
\end{tabularx}\par
\vspace{6mm}
\lead{Operating values to confirm}
\vspace{-2mm}
\(R_{\rm useful}=\dot N_{\rm pol}\,n_t\,\sigma_{\rm acc}\,\epsilon\,f_{\rm live}\)\par
\vspace{3mm}
\smalltext
Agree on beam energy, intensity, target thickness, and the required $p_{yy}$ uncertainty within a specified time.\par
\vspace{2mm}
\textbf{Higher useful counts shorten the monitoring interval.} Analyzing power and backgrounds also determine the precision.
\source{The preceding precision plots use provisional rate inputs from the current configuration. Confirm delivered intensity, target thickness, and measured response before setting the monitoring interval.}
\note{\notesix}
\end{frame}
\fi
\end{document}
