...
Run \texttt{make} in a topic directory to compile its documents. Each topic keeps intermediate files in its own \path{build/} and copies finished PDFs beside the TeX sources; the [PDF] links open these local outputs. For example, run \texttt{make -C docs/beamline\_info/afterSRC/polarimeter\_rationale}. Use \texttt{make -C docs} to build the registered reading list and both guides, \texttt{make -C docs guides} for the guides alone, and \texttt{make -C docs verify} to check navigation. Guide intermediates live in \path{docs/index/build/}; the entry PDFs remain \path{docs/index_zh.pdf} and \path{docs/index_en.pdf}. Keep the docs directory together to preserve relative links.
\endgroup
\end{document}
